...
import { calculateOrderPricing, type CustomerService } from "@/domain/order";
...
function StationScreen() {
  ...
  const loadStations = () => {
    ...
    const controller = new AbortController();
    requestRef.current = controller;
    dispatch({ type: "STATIONS_LOADING" });
    void listStations(controller.signal)
      .then((stations) => dispatch({ type: "STATIONS_READY", stations }))
      .catch((error) => {
        if (error instanceof DOMException && error.name === "AbortError") return;
        dispatch({ type: "STATIONS_ERROR", message: "Approved stations could not be loaded. Please try again." });
      });
  };
  useEffect(() => { if (state.stationsStatus === "idle") loadStations(); return () => requestRef.current?.abort(); }, []);
  ...
  return <Screen><FlowHead step="4 of 9 · Choose station" back={() => dispatch({ type: "GO_TO", screen: "product" })} /><div className="journey-narrow"><h2>Where should we fit them?</h2><p className="journey-sub">Choose an approved TyreLink fitting station.</p>{state.stationsStatus === "loading" && <div className="journey-card" role="status">Loading approved stations…</div>}{state.stationsStatus === "error" && <div className="journey-error" role="alert"><strong>Stations unavailable</strong><span>{state.stationsError}</span><button className="journey-button small" onClick={loadStations}>Try again</button></div>}{state.stationsStatus === "ready" && (state.stations.length ? <div className="journey-product-list">{state.stations.map((station) => <button className="journey-station" key={station.id} onClick={() => dispatch({ type: "SELECT_STATION", station })}><span><strong>{station.trading_name}</strong><small>{station.address_line}, {station.city} · {station.fitting_bays} bays</small></span><strong>GHS {station.fittingPrice.toLocaleString()} / tyre</strong></button>)}</div> : <div className="journey-card">No approved fitting stations are available yet.</div>)}</div></Screen>;
}
...
function SlotScreen() {
  ...
  const loadSlots = () => {
    ...
    void listSlots(station.id, controller.signal)
      .then((slots) => dispatch({ type: "SLOTS_READY", slots }))
      .catch((error) => {
        if (error instanceof DOMException && error.name === "AbortError") return;
        dispatch({ type: "SLOTS_ERROR", message: "Appointment slots could not be loaded. Please try again." });
      });
  };
  useEffect(() => { loadSlots(); return () => requestRef.current?.abort(); }, [station?.id]);
  if (!station) return <StationScreen />;
  return <Screen><FlowHead step="6 of 9 · Book a time" back={() => dispatch({ type: "GO_TO", screen: "services" })} /><div className="journey-narrow"><h2>When suits you?</h2><p className="journey-sub">Available slots at {station.trading_name}.</p>{state.slotsStatus === "loading" && <div className="journey-card" role="status">Loading available appointments…</div>}{state.slotsStatus === "error" && <div className="journey-error" role="alert"><strong>Appointments unavailable</strong><span>{state.slotsError}</span><button className="journey-button small" onClick={loadSlots}>Try again</button></div>}{state.slotsStatus === "ready" && (state.slots.length ? <div className="journey-slot-grid">{state.slots.map((slot) => <button className="journey-slot" key={slot.id} onClick={() => dispatch({ type: "SELECT_SLOT", slot })}>{new Date(slot.starts_at).toLocaleString("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "numeric", minute: "2-digit" })}</button>)}</div> : <div className="journey-card">No appointment slots are available for this station yet.</div>)}</div></Screen>;
}
...
function ReviewScreen() {
  const { state, dispatch } = useJourney();
  const pricing = calculateOrderPricing({ quantity: state.quantity, tyreUnitPrice: state.selectedTyre?.unitPrice ?? 0, fittingUnitPrice: state.selectedStation?.fittingPrice ?? 0, services: state.selectedServices });
  return <Screen><FlowHead step="8 of 9 · Confirm" back={() => dispatch({ type: "GO_TO", screen: "details" })} /><div className="journey-narrow"><h2>Confirm your order.</h2><p className="journey-sub">Review the itemised total. A 10% deposit is required to secure the booking.</p><div className="journey-card"><div className="journey-summary-row"><span>Customer</span><strong>{state.customerName}</strong></div><div className="journey-summary-row"><span>Tyres</span><strong>{state.selectedTyre?.brand} {state.selectedTyre?.model} × {state.quantity}</strong></div><div className="journey-summary-row"><span>Tyres subtotal</span><strong>GHS {pricing.tyreSubtotal.toLocaleString()}</strong></div><div className="journey-summary-row"><span>Fitting · {state.quantity} tyres</span><strong>GHS {pricing.fittingSubtotal.toLocaleString()}</strong></div>{state.selectedServices.map((service) => <div className="journey-summary-row" key={service.id}><span>{service.name}</span><strong>GHS {(service.price * (service.priceUnit === "per tyre" ? state.quantity : 1)).toLocaleString()}</strong></div>)}<div className="journey-summary-row"><span>Station</span><strong>{state.selectedStation?.trading_name}</strong></div><div className="journey-summary-row"><span>Appointment</span><strong>{state.selectedSlot ? new Date(state.selectedSlot.starts_at).toLocaleString("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }) : "Selected slot"}</strong></div><div className="journey-summary-total"><span>Order total</span><strong>GHS {pricing.total.toLocaleString()}</strong></div><div className="journey-deposit"><span>Deposit due now · 10%</span><strong>GHS {pricing.deposit.toLocaleString()}</strong><small>Remaining balance at fitting: GHS {pricing.balance.toLocaleString()}</small></div><button className="journey-button" onClick={() => dispatch({ type: "GO_TO", screen: "payment" })}>Continue to payment →</button></div></div></Screen>;
}
...
function PaymentScreen() {
  ...
  return <Screen><FlowHead step="9 of 9 · Deposit payment" back={() => dispatch({ type: "GO_TO", screen: "review" })} /><div className="journey-narrow"><h2>Pay your booking deposit.</h2><p className="journey-sub">A 10% deposit of <strong>GHS {pricing.deposit.toLocaleString()}</strong> secures your appointment. The balance of GHS {pricing.balance.toLocaleString()} is due at fitting.</p><div className="journey-card">{methods.map(([method, label, copy]) => <button className={`journey-payment-option${state.paymentMethod === method ? " selected" : ""}`} key={method} onClick={() => dispatch({ type: "SET_PAYMENT_METHOD", method })}><span><strong>{label}</strong><small>{copy}</small></span><span>{state.paymentMethod === method ? "✓ Selected" : "Select"}</span></button>)}<button className="journey-button" onClick={() => dispatch({ type: "CREATE_ORDER" })}>Confirm {state.paymentMethod} deposit →</button></div></div></Screen>;
}
